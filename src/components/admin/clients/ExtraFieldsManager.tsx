  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent
          className={`max-w-4xl max-h-[85vh] overflow-hidden flex flex-col ${isRTL ? "dir-rtl" : ""}`}
          dir={isRTL ? "rtl" : "ltr"}
        >
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {t("admin.extraFieldsManager.title")}:{" "}
              <span className="text-primary underline">{t("admin.extraFieldsManager.subtitle")}</span>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 flex items-start gap-3">
            ...
            <p className="text-sm text-foreground">{t("admin.extraFieldsManager.description")}</p>
          </div>
          ...
          <div className="flex-1 overflow-auto border rounded-lg">
            <table className="w-full">
              <thead className="sticky top-0 z-10">
                <tr>
                  <th
                    className={`${isRTL ? "text-right" : "text-left"} p-3 text-sm font-medium border-b bg-slate-100 dark:bg-slate-900`}
                  >
                    #
                  </th>
                  <th
                    className={`${isRTL ? "text-right" : "text-left"} p-3 text-sm font-medium border-b bg-slate-100 dark:bg-slate-900`}
                  >
                    {t("admin.extraFieldsManager.field")}
                  </th>
                  <th
                    className={`${isRTL ? "text-right" : "text-left"} p-3 text-sm font-medium border-b bg-slate-100 dark:bg-slate-900`}
                  >
                    {t("admin.extraFieldsManager.informationType")}
                  </th>
                  <th
                    className={`${isRTL ? "text-right" : "text-left"} p-3 text-sm font-medium border-b bg-slate-100 dark:bg-slate-900`}
                  >
                    {t("admin.extraFieldsManager.defaultValue")}
                  </th>
                  <th
                    className={`${isRTL ? "text-right" : "text-left"} p-3 text-sm font-medium border-b bg-slate-100 dark:bg-slate-900`}
                  >
                    {t("admin.extraFieldsManager.createdAt")}
                  </th>
                    ...
                    <tr key={field.id} className="border-t hover:bg-muted/30 transition-colors">
                      <td className="p-3 text-sm">
                        <div className="flex items-center gap-1">
                          <span>{index + 1}</span>
                          {field.isSearchable && <Search className="h-3 w-3 text-muted-foreground" />}
                        </div>
                      </td>
                      <td className="p-3">
                        <div>
                          <div className="font-medium text-sm flex items-center gap-2">
                            ...
                            {field.allowOwnerEdit && <Edit2 className="h-3 w-3 text-amber-500" />}
                          </div>
                        ...
                        <div>
                          ...
                          {field.type === "options" && field.options && (
                            <div className="text-xs text-muted-foreground mt-1">
                              {field.options.map((o) => (
                                <span key={o.value} className="mr-2">
                                  ⊙ {o.label}
                                </span>
                              ))}
                            </div>
                      ...
                      <td className="p-3 text-sm text-muted-foreground">{field.defaultValue?.toString() || "--"}</td>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent
          className={`max-w-xl max-h-[90vh] overflow-y-auto ${isRTL ? "dir-rtl" : ""}`}
          dir={isRTL ? "rtl" : "ltr"}
        >
          <DialogHeader>
            <DialogTitle>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                dir={isRTL ? "rtl" : "ltr"}
              />
              <p className="text-xs text-muted-foreground">{t("admin.extraFieldsManager.defaultValueHelper")}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                dir={isRTL ? "rtl" : "ltr"}
              />
              <p className="text-xs text-muted-foreground">{t("admin.extraFieldsManager.codeHelper")}</p>
            </div>
            ...
            <div className="col-span-2 space-y-3">
              <div className="flex items-start gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData({ ...formData, allowOwnerEdit: checked as boolean })}
                />
                <div>
              ...
              <div className="flex items-start gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData({ ...formData, isSearchable: checked as boolean })}
                />
                <div>
                  <Label htmlFor="isSearchable" className="cursor-pointer font-medium">
                    {t("admin.extraFieldsManager.isSearchable")}
                  </Label>
                  <p className="text-xs text-muted-foreground">Show this field in filtering/search options</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">This field must be filled when creating a client</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <AlertDialog open={!!deleteField} onOpenChange={() => setDeleteField(null)}>
        <AlertDialogContent dir={isRTL ? "rtl" : "ltr"}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("admin.extraFieldsManager.deleteMessage").replace("{field}", deleteField?.title || "")}{" "}
              {deleteField?.title}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
